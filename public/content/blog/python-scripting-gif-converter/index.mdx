...
File GIF merupakan file gambar yang bisa bergerak. Terkadang, kita tidak ingin mengupload file dalam bentuk video. Nah... Kita bisa gunain format GIF ini. Tapi, kalau kita udah ada file video nya, berarti kita perlu meng-konversi file video tersebut ke format GIF. Disini aku bakal jelasin gimana caranya nge-konversi file video ke format GIF, dan aku juga bakal bikin scripting nya supaya lebih gampang meng-konversi.
...
Untuk meng-konversi dari file video ke format GIF, kita bisa menggunakan bantuan `ffmpeg`. Jika kalian belum punya `ffmpeg`, kalian bisa ikuti tutorial instalasi nya di [website ini](https://www.hostinger.com/tutorials/how-to-install-ffmpeg). Command `ffmpeg` ini mempunyai banyak opsi. Untuk mengubah video mp4 bernama `input.mp4` ke bentuk file GIF bernama `output.gif`, kita bisa menjalankan command berikut:
...
<Callout title="Cobain" status="info">
	Sebelum lanjut, kalian perlu nyobain dulu command diatas. Kalau ternyata command tersebut berhasil mengkonversi file video menjadi file GIF, kalian bisa lanjut ke bagian selanjutnya.
</Callout>
...
Sekarang kita coba konversi video berformat WEBM ke format GIF. Disini saya sudah ada video bernama `once-upon-a-time.mp4` yang saya ambil dari [video youtube ini](https://youtu.be/hY8WyRwpPz8?si=ltN2HgdYaa_a5pd8). Video nya seperti ini:

<video width="320" height="240" className="w-full" controls preload="none">
