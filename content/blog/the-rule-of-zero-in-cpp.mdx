---
icon: 📏
title: The Rule of Zero pada Bahasa C++
description: Penjelasan mengenai konsep rule of zero pada Bahasa C++ yang disertai dengan contoh penggunaan nya
date: 2022-04-17
tags:
  - cpp
  - oop
...
class bar {
public:
	std::unique_ptr<int> p = std::make_unique<int>(5); std::unique_ptr<int> p = std::make_unique<int>(5); std::unique_ptr<int> p = std::make_unique<int>(5); std::unique_ptr<int> p = std::make_unique<int>(5); std::unique_ptr<int> p = std::make_unique<int>(5);
};
